...
def pipeline(config):
    ...
    if config.CHECK_MOSAICED:
        for rev in revs:
            if check_mosaic_complete(config, rev):
                if not config.OFFLOAD:
                    already_processed += 1
                    continue
                else:
                    if check_archive_complete(config, rev):
                        already_processed += 1
                        continue
    ...
    if config.CHECK_MOSAICED:
        progress.update_status(
            "to_process", f"{already_processed} revolutions already processed"
        )
    else:
        progress.update_status(
            "to_process",
            f"{already_processed} revolutions already processed (check ignored)",
        )
    ...
    for i, rev in enumerate(to_process):
        ...
        if config.DO_PROCESSING:
            progress.update_status(
                "heading", f"Processing revolution {rev} ({i + 1}/{len(to_process)})"
            )
        ...
        if config.OFFLOAD:
            ...
            def _run_offload(rev=rev):
                try:
                    offload_result["n"], offload_result["time"] = offload(
                        rev,
